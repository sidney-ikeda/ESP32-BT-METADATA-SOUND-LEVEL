...
void BluetoothA2DPSink::set_metadata_reader(void (*callBack)(uint8_t, const uint8_t*)){
    this->metadata_reader = callBack;
}
...
void  BluetoothA2DPSink::av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_A2D_CONNECTION_STATE_EVT: {
        ...
        if (a2d->conn_stat.state == ESP_A2D_CONNECTION_STATE_DISCONNECTED) {
			...
			//	connectToLastDevice();
			//} else //if (( *lastBda != NULL ) && a2d->conn_stat.disc_rsn == ESP_A2D_DISC_RSN_NORMAL ){ <- //https://github.com/pschatzmann/ESP32-A2DP/pull/27
    ...
    case ESP_A2D_AUDIO_STATE_EVT: {
        ...
        { 
			...
			i2s_start(i2s_port); 
               ...
               ESP_A2D_AUDIO_STATE_STOPPED == a2d->audio_stat.state)  // <- https://github.com/pschatzmann/ESP32-A2DP/issues/24
		{ 
			i2s_stop(i2s_port);
			//av_new_track(); //included by toli
		}
        break;
    }
...
void  BluetoothA2DPSink::av_hdl_avrc_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
        if (metadata_reader!=NULL){ // <- https://github.com/pschatzmann/ESP32-A2DP/issues/29
            metadata_reader(rc->meta_rsp.attr_id, rc->meta_rsp.attr_text);
        }
...
void BluetoothA2DPSink::sendCommand(BT_CMND cmnd){
	switch(cmnd){
        esp_err_t err;
		case PLAY:
			err = esp_avrc_ct_send_passthrough_cmd(0, ESP_AVRC_PT_CMD_PLAY, ESP_AVRC_PT_CMD_STATE_PRESSED);
            //if (err == ESP_OK) {esp_avrc_ct_send_passthrough_cmd(0, ESP_AVRC_PT_CMD_PLAY, ESP_AVRC_PT_CMD_STATE_RELEASED);}
			break;
			
		case PAUSE:
			err = esp_avrc_ct_send_passthrough_cmd(0, ESP_AVRC_PT_CMD_PAUSE, ESP_AVRC_PT_CMD_STATE_PRESSED);
            //if (err == ESP_OK) {esp_avrc_ct_send_passthrough_cmd(2, ESP_AVRC_PT_CMD_PAUSE, ESP_AVRC_PT_CMD_STATE_RELEASED);}
			break;
			
		case STOP:
			err = esp_avrc_ct_send_passthrough_cmd(0, ESP_AVRC_PT_CMD_STOP, ESP_AVRC_PT_CMD_STATE_PRESSED);
            //if (err == ESP_OK) {esp_avrc_ct_send_passthrough_cmd(1, ESP_AVRC_PT_CMD_STOP, ESP_AVRC_PT_CMD_STATE_RELEASED);}
			break;
			
		case NEXT:
			err = esp_avrc_ct_send_passthrough_cmd(0, ESP_AVRC_PT_CMD_FORWARD, ESP_AVRC_PT_CMD_STATE_PRESSED);
            if (err == ESP_OK) {esp_avrc_ct_send_passthrough_cmd(3, ESP_AVRC_PT_CMD_FORWARD, ESP_AVRC_PT_CMD_STATE_RELEASED);}
			break;
			
		case PREV:
			err = esp_avrc_ct_send_passthrough_cmd(0, ESP_AVRC_PT_CMD_BACKWARD, ESP_AVRC_PT_CMD_STATE_PRESSED);
            if (err == ESP_OK) {esp_avrc_ct_send_passthrough_cmd(4, ESP_AVRC_PT_CMD_BACKWARD, ESP_AVRC_PT_CMD_STATE_RELEASED);}
			break;
